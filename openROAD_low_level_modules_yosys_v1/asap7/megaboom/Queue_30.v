module Queue_30(
  ...
  output [2:0] io_deq_bits_addr_lo,
...
`ifdef RANDOMIZE
  ...
  `ifdef RANDOMIZE_MEM_INIT
  for (initvar = 0; initvar < 2; initvar = initvar+1)
    ram_sink[initvar] = _GEN_4[1:0];
  `endif
  _GEN_5 = {1{$random}};
  `ifdef RANDOMIZE_MEM_INIT
  for (initvar = 0; initvar < 2; initvar = initvar+1)
    ram_addr_lo[initvar] = _GEN_5[2:0];
  `endif
  _GEN_6 = {2{$random}};
  `ifdef RANDOMIZE_MEM_INIT
  for (initvar = 0; initvar < 2; initvar = initvar+1)
